...
import { toCodeBuddyEnv } from './env-builder';
import { findCodeBuddyBinary } from './platform';
import { findGitBash, getExpandedPath } from '../platform';
import { getRuntimeSessionId, setRuntimeSessionId } from './session';
  ...
  return new ReadableStream<string>({
    async start(controller) {
      ...
      try {
        ...
        if (systemPrompt) {
          // Keep plain string form for SDK type compatibility.
        ...
        if (thinking) {
          queryOptions.thinking = thinking as Options['thinking'];
        }
        if (effort) {
          // CodeBuddy accepts 'xhigh'; cast to keep compatibility with Claude SDK types.
          queryOptions.effort = (effort === 'max' ? 'xhigh' : effort) as unknown as Options['effort'];
        }
        if (outputFormat) {
